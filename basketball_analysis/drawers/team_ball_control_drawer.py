...
class TeamBallControlDrawer:
    ...
    def get_team_ball_control(self, player_assignment, ball_aquisition):
        team_ball_control = []
        for player_assigment_frame, ball_aquisition_frame in zip(player_assignment, ball_aquisition):
            if ball_aquisition_frame == -1:
                team_ball_control.append(-1)
                continue

            if ball_aquisition_frame not in player_assigment_frame:
                team_ball_control.append(-1)
                continue

            if player_assigment_frame[ball_aquisition_frame] == 1:
                team_ball_control.append(1)
            else:
